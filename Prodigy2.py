...
class ImageEncryptionTool:
    ...
    def create_widgets(self):
        ...
        self.canvas = tk.Canvas(self.root, width=500, height=500, bg="#345621")
        self.canvas.grid(row=0, column=0, columnspan=4, padx=10, pady=10, sticky="nsew")

        # Button to select an image
        self.btn_select = tk.Button(self.root, text="Select Image",
                                    command=self.select_image, bg="#0080ff", fg="white")
        self.btn_select.grid(row=1, column=0, padx=5, pady=10, sticky="ew")

        # Button to encrypt the image
        self.btn_encrypt = tk.Button(
            self.root, text="Encrypt Image", command=self.encrypt_image, bg="#4CAF50", fg="white")
        self.btn_encrypt.grid(row=1, column=1, padx=5, pady=10, sticky="ew")

        # Button to decrypt the image
        self.btn_decrypt = tk.Button(
            self.root, text="Decrypt Image", command=self.decrypt_image, bg="#FF5733", fg="white")
        self.btn_decrypt.grid(row=1, column=2, padx=5, pady=10, sticky="ew")

        # Button to download the image
        self.btn_download = tk.Button(
            self.root, text="Download Image", command=self.download_image, bg="#ff9933", fg="white")
        self.btn_download.grid(row=1, column=3, padx=5, pady=10, sticky="ew")
